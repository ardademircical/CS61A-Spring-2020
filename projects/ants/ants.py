...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            elif isinstance(self, ContainerAnt) and not self.contained_ant and  not isinstance(place.ant, ContainerAnt):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        # while place_holder != beehive:
        #     if random_or_none(place_holder.bees):
        ...
        if self.min_range > 0:
            for i in range(self.min_range):
                if place_holder != beehive:
                    place_holder = place_holder.entrance

        while n <= (self.max_range - self.min_range) and place_holder != beehive:
                if random_or_none(place_holder.bees):
                    return random_or_none(place_holder.bees)
                else:
                    place_holder = place_holder.entrance
...
class HungryAnt(Ant):
    ...
    def __init__(self, armor=1):
        ...
        self.digesting = self.time_to_digest - 3
        self.armor = armor 
        # END Problem 6
    ...
    def eat_bee(self, bee):
        ...
        bee_armor = bee.armor  
        Insect.reduce_armor(bee, bee_armor)
    ...
    def action(self, gamestate):
        ...
        if self.digesting > 0:
            ...
            if random_or_none(self.place.bees):
                self.digesting = self.time_to_digest
                s_bee = random_or_none(self.place.bees)
                return self.eat_bee(s_bee)
...
class BodyguardAnt(ContainerAnt):
    ...
    def __init__(self, *args, armor=2):
        Ant.__init__(self, *args, armor=2)
        self.contained_ant = None
        self.armor = armor 
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.impostor:
            ...
            observed = self.place.exit

            while observed:
                
                if observed.ant:

                    if isinstance(observed.ant, ContainerAnt):
                        if observed.ant not in QueenAnt.upgraded_ants:
                            observed.ant.damage *= 2
                            QueenAnt.upgraded_ants.append(observed.ant)

                        if observed.ant.contained_ant:

                            if observed.ant.contained_ant not in QueenAnt.upgraded_ants:
                                observed.ant.contained_ant.damage *= 2
                                QueenAnt.upgraded_ants.append(observed.ant.contained_ant)
...
def apply_effect(effect, bee, duration):
    ...
    altered_action = effect(bee.action, bee)

    def effected(gamestate):
        nonlocal duration
        if duration > 0:
            duration -= 1
            return altered_action(gamestate)
        else:
            bee.direction = True
            bee.scared = True 
            return real_action(gamestate)
