...
def free_bacon(score):
    ...
    rival_cube = pow(score, 3)
    f_digit = 0   
    s_digit = 0
    while rival_cube > 0:
        s_digit = ((rival_cube % 10) + s_digit)
        rival_cube = rival_cube // 10
        f_digit = (-(rival_cube % 10) + f_digit)
        rival_cube = rival_cube // 10 
    return 1 + abs(s_digit + f_digit)
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        if who == 0:
            ...
            newscore0 = take_turn(strat0, score1, dice)
            score0 += newscore0
            difference0 = abs(strat0 - oldscore0)
            if difference0 == 2:
                if feral_hogs:
                    score0 += 3
                else:
            ...
            newscore1 = take_turn(strat1, score0, dice)
            score1 += newscore1
            difference1 = abs(strat1 - oldscore1)
            if difference1 == 2:
                if feral_hogs:
                    score1 += 3
                else:
...
def announce_highest(who, prev_high=0, prev_score=0):
    ...
    def say(score0, score1): 
        ...
        return announce_highest(who, local_high, localprevs)
...
def make_averaged(g, num_samples=1000):
    ...
    def averagetor(*args):
        score_t = 0
        roll_time = 0
        while roll_time < num_samples: 
            score_t = g(*args) + score_t
            roll_time += 1
            if roll_time == 0:
                s_avg = 0
            else: 
                s_avg = score_t / roll_time
        return s_avg
    return averagetor
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    num_rolls = 1
    while num_rolls <= 10:
        rsvg = make_averaged(roll_dice, num_samples)(num_rolls, dice)
        if rsvg > hsvg:
            hsvg = rsvg
            high = num_rolls
        num_rolls += 1
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=6):
    ...
    post_free_bacon = free_bacon(opponent_score)

    if new_score < opponent_score and is_swap(new_score, opponent_score): #favorable swap
        return 0
    elif new_score > opponent_score and is_swap(new_score, opponent_score):
        return num_rolls
    elif post_free_bacon >= margin: #score at least margin
        return 0
    elif score > opponent_score:
        return num_rolls
    elif new_score > opponent_score and is_swap(new_score, opponent_score):
        return num_rolls
    elif opponent_score < score and is_swap(new_score, opponent_score) and new_score < opponent_score:
        return num_rolls
    else: #detrimental swaps
        return num_rolls
    # END PROBLEM 11
